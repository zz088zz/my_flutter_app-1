...
class _HomeScreenState extends State<HomeScreen>
  ...
  Future<void> _refreshEnergyData() async {
    ...
    if (timeSinceLastRefresh < _minRefreshInterval) {
      print(
        'Throttling energy refresh: Last refresh was ${timeSinceLastRefresh.inMilliseconds}ms ago',
      );
      return;
    }
    ...
    try {
      ...
      try {
        ...
        print(
          'Energy calculation complete: $totalEnergy kWh, CO2: $co2Saved kg',
        );
    ...
    if (authService.currentUser != null) {
      try {
        ...
        if (!forceRefresh && timeSinceLastRefresh < _minRefreshInterval) {
          print(
            'Throttling refresh: Last refresh was ${timeSinceLastRefresh.inMilliseconds}ms ago',
          );
  ...
  Widget build(BuildContext context) {
    ...
    final String userInfo =
        currentUser != null ? 'Welcome to EV Charging!' : 'Welcome';
  ...
  Widget _buildChargingStationCard({required ChargingStation station}) {
    ...
    final Set<String> connectorTypes =
        station.chargers.map((c) => c.type).toSet();
          ...
          children: [
            ...
            Align(
              ...
              child: ElevatedButton(
                onPressed: isAvailable ? () => _reserveStation(station) : null,
  ...
  Future<void> _runReservationDiagnostics(String userId) async {
    ...
    try {
      ...
      print(
        'Found ${reservationsQuery.docs.length} reservations for user $userId',
      );

      // Check for any pending or confirmed reservations
      int pendingCount = 0;
      ...
      print('Reservation status summary:');
      print('- Pending: $pendingCount');
      print('- Confirmed: $confirmedCount');
      print('- Completed: $completedCount');

      // Check for any overdue reservations
      final now = DateTime.now();
      for (var doc in reservationsQuery.docs) {
        final data = doc.data();
        if (data['status'] == 'confirmed') {
          ...
          if (now.isAfter(endTime)) {
            print('WARNING: Overdue reservation found: ${doc.id}');
            print('- Started: ${startTime.toIso8601String()}');
            print('- Should have ended: ${endTime.toIso8601String()}');
          }
        }
      }
    } catch (e) {

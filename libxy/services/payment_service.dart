...
class PaymentService with ChangeNotifier {
  ...
  Future<void> loadUserPaymentMethods(dynamic userId) async {
    ...
    try {
      print(
        'Loading payment methods for user ID: $userId (type: ${userId.runtimeType})',
      );

      // Ensure userId is a string
      final String userIdString = userId.toString();
      print('Converted user ID to string: $userIdString');

      final query =
          await FirebaseFirestore.instance
              .collection('payment_methods')
              .where('user_id', isEqualTo: userIdString)
              .get();
      print(
        'Query returned ${query.docs.length} documents for user $userIdString',
      );
      ...
      _paymentMethods =
          query.docs.map((doc) {
            final map = doc.data();
            map['id'] = doc.id;
            print('Processing payment method: ${doc.id} with data: $map');
      ...
      for (var method in _paymentMethods) {
        print(
          '- ${method.cardType}: ${method.maskedCardNumber} (Default: ${method.isDefault})',
        );
      }
    } catch (e) {
  ...
  Future<PaymentMethod?> addPaymentMethod(PaymentMethod paymentMethod) async {
    try {
      print('Adding payment method: ${paymentMethod.toFirestoreMap()}');
      ...
      if (paymentMethod.isDefault) {
        final query =
            await FirebaseFirestore.instance
                .collection('payment_methods')
                .where('user_id', isEqualTo: paymentMethod.userId)
                .get();
      ...
      final docRef = await FirebaseFirestore.instance
          .collection('payment_methods')
          .add(paymentMethod.toFirestoreMap());
  ...
  Future<bool> deletePaymentMethod(String methodId, dynamic userId) async {
    try {
      ...
      print(
        'Payment method found at index $methodIndex: ${_paymentMethods[methodIndex].toMap()}',
      );
      await FirebaseFirestore.instance
          .collection('payment_methods')
          .doc(methodId)
  ...
  Future<bool> setDefaultPaymentMethod(String methodId, dynamic userId) async {
    try {
      ...
      if (methodIndex == -1) {
        ...
        print(
          'Available methods: ${_paymentMethods.map((m) => 'ID: ${m.id}, Type: ${m.cardType}').join(', ')}',
        );
        return false;
      }
  ...
  Future<bool> updatePaymentMethod(PaymentMethod method) async {
    try {
      ...
      if (method.isDefault) {
        ...
        final currentMethodIndex = _paymentMethods.indexWhere(
          (m) => m.id == method.id,
        );
        final wasDefault =
            currentMethodIndex != -1
                ? _paymentMethods[currentMethodIndex].isDefault
                : false;
        ...
        if (!wasDefault) {
          final query =
              await FirebaseFirestore.instance
                  .collection('payment_methods')
                  .where('user_id', isEqualTo: method.userId)
                  .get();
          final batch = FirebaseFirestore.instance.batch();
          for (var doc in query.docs) {
            if (doc.id != method.id) {

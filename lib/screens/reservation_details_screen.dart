...
class _ReservationDetailsScreenState extends State<ReservationDetailsScreen> {
  ...
  Future<void> _checkAvailabilityForTimeSlot() async {
    ...
    try {
      ...
      print('Checking availability for ${startDateTime.toIso8601String()} with duration $totalMinutes minutes');
      ...
      for (final charger in widget.station.chargers) {
        print('Charger ${charger.id}: ${charger.name} (${charger.type}, ${charger.power}kW) - ${charger.isAvailable ? "Available" : "Unavailable"}');
      }
      ...
      for (final charger in widget.station.chargers) {
        ...
        if (isAvailable) {
          ...
          print('Charger ${charger.name} is not available for the selected time slot');
          notAvailableCount++;
        }
      }
      ...
      if (mounted) {
        setState(() {
          ...
          if (previouslySelectedCharger != null && availableChargers.any((c) => c.id == previouslySelectedCharger.id)) {
            // Keep the same charger selected if it's still available
            _selectedCharger = availableChargers.firstWhere((c) => c.id == previouslySelectedCharger.id);
            print('Restored previously selected charger: ${_selectedCharger?.name}');
            _previousChargerBecameUnavailable = false;
            ...
            if (availableChargers.isNotEmpty) {
              ...
              print('Previously selected charger became unavailable, selected first available: ${_selectedCharger?.name}');
            } else {
              _selectedCharger = null;
              print('Previously selected charger became unavailable and no others are available');
            }
          } else if (availableChargers.isNotEmpty) {
            // First time selecting or no previous selection
            _selectedCharger = availableChargers.first;
            print('Selected first available charger: ${_selectedCharger?.name}');
          } else {
  ...
  void _updateTimeSlotAndCheckAvailability({DateTime? date, TimeOfDay? time, int? hours, int? minutes}) {
    setState(() {
  ...
  double get _chargingFee {
    if (_selectedCharger != null) {
      ...
      double estimatedEnergyKWh = (_selectedCharger!.power * _totalDurationMinutes) / 60.0;
      ...
      double estimatedEnergyKWh = (averagePowerKW * _totalDurationMinutes) / 60.0;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Charger Reservation'),
        elevation: 0,
      ),
      body: _isLoading
                      ...
                      children: [
                        ...
                        Consumer<VehicleService>(
                          builder: (context, vehicleService, child) {
                            ...
                            if (vehicles.isEmpty) {
                              return const Text('No vehicles added. Please add a vehicle in your account.');
                            }
  ...
  Widget _buildChargerSelector() {
    ...
    final availableChargers = _availableChargersForTimeSlot.toList()
      ...
      return Container(
        ...
        child: Column(
          ...
          children: [
            ...
            if (_previousChargerBecameUnavailable && _previousChargerName != null) ...[
              const SizedBox(height: 8),
              Row(
                children: [
        ...
        Container(
          ...
          child: DropdownButtonHideUnderline(
            child: DropdownButton<Charger>(
              ...
              items: availableChargers.map((charger) {
                String chargerType = charger.type == 'AC' ? 'AC' : 'DC Fast';
                String powerOutput = '${charger.power.toStringAsFixed(1)} kW';
            ...
            child: Row(
              children: [
                Icon(
                  ...
                  color: _selectedCharger!.type == 'AC' ? Colors.blue : Colors.orange,
                  size: 16,
                ),
                const SizedBox(width: 4),
                Text(

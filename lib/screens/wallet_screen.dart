...
class _WalletScreenState extends State<WalletScreen> {
    ...
    Future.delayed(const Duration(seconds: 15), () {
      if (mounted) {
        ...
        if (walletService.isLoading) {
          print('WARNING: Wallet loading timeout - forcing completion');
          walletService.forceStopLoading();
        }
      }
    });
  }
  ...
  Future<void> _loadWalletData() async {
    try {
      print('Starting wallet data load...');
      ...
      // Try to load wallet data directly first
      bool walletLoaded = false;
      try {
        await walletService.loadWalletForUser(userId).timeout(const Duration(seconds: 20));
        walletLoaded = true;
      } catch (e) {
        print('Warning: Wallet loading failed: $e');
        walletService.forceStopLoading();
      }
      
      // Only test connectivity if wallet loading failed
      if (!walletLoaded) {
        final isConnected = await walletService.testFirebaseConnection();
        if (!isConnected) {
          print('ERROR: Firebase connection test failed');
          throw Exception('No internet connection or Firebase unavailable');
        } else {
          throw Exception('Unable to load wallet data. Please try again.');
        }
      }
      
      // Load payment methods with timeout protection
      try {
        await paymentService.loadUserPaymentMethods(userId).timeout(const Duration(seconds: 10));
      } catch (e) {
        print('Warning: Payment methods loading failed: $e');
        // Continue without payment methods - wallet can still work
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error loading wallet data: ${e.toString()}'),
  ...
  Future<void> _topUpWallet(double amount) async {
    ...
    try {
      ...
      if (success) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Successfully added RM ${amount.toStringAsFixed(2)} to your wallet')),
          );
  ...
  Widget build(BuildContext context) {
    ...
    final recentTransactions = walletService.transactions.where((tx) => 
      tx.transactionSource == 'wallet' || tx.transactionSource == null
    ).toList();
                    ...
                    children: [
                      if (paymentMethods.isEmpty)
                        const Padding(
                          ...
                          child: Text('No saved cards. Add cards in your account settings.'),
                        )
                      else

...
class StationService with ChangeNotifier {
  ...
  Future<void> updateStationDistances() async {
    try {
      ...
      if (permission == LocationPermission.deniedForever) {
        print('Location permissions are permanently denied. Using default distances.');
        _setDefaultDistances();
        return;
      }
      ...
      for (int i = 0; i < _stations.length; i++) {
        ...
        print('Station ${station.name}: ${formattedDistance} (${station.latitude}, ${station.longitude})');
  ...
  double _parseDistance(String distance) {
    if (distance.contains('km')) {
      ...
      double meters = double.tryParse(distance.replaceAll(' m', '')) ?? 999000.0;
    ...
    for (var station in _stations) {
      for (var charger in station.chargers) {
        if (charger.id == chargerId) {
          print('Found charger in memory: ${charger.name} (${charger.type} ${charger.power}kW)');
          return charger;
        }
      }
    }
  ...
  Future<Charger?> getChargerByIdAsync(String chargerId) async {
    ...
    try {
      ...
      final doc = await FirebaseFirestore.instance.collection('chargers').doc(chargerId).get();
      if (doc.exists) {
        print('Found charger in Firestore: ${doc.data()}');
        return Charger.fromMap(doc.data()!..['id'] = doc.id);
      }
      print('Charger with ID $chargerId not found in Firestore, checking memory cache');
  ...
  Future<void> loadStations() async {
    ...
    notifyListeners();
    try {
      final stationQuery = await FirebaseFirestore.instance.collection('charging_stations').get();
      final List<ChargingStation> newStations = [];
      for (var stationDoc in stationQuery.docs) {
        final stationId = stationDoc.id;
        final chargerQuery = await FirebaseFirestore.instance.collection('chargers').where('station_id', isEqualTo: stationId).get();
        final chargers = chargerQuery.docs.map((doc) => Charger.fromMap(doc.data()!..['id'] = doc.id)).toList();
        final station = ChargingStation.fromMap(stationDoc.data()!..['id'] = stationId);
      ...
      for (var station in _stations) {
        print('Loaded station: ${station.name} with ${station.chargers.length} chargers');
      }
  ...
  List<ChargingStation> getAvailableStations() {
    return _stations.where((station) => station.availableSpots > 0).toList();
  }
  
  Future<ChargingStation?> getStationById(String id) async {
    final stationData = await FirebaseFirestore.instance.collection('charging_stations').doc(id).get();
    if (!stationData.exists) return null;
    return ChargingStation.fromMap(stationData.data()!..['id'] = stationData.id);
  }
  
  Future<bool> updateStationAvailability(String id, int availableSpots) async {
    try {
      final stationRef = FirebaseFirestore.instance.collection('charging_stations').doc(id);
        ...
        for (int i = 0; i < existingChargers.length; i++) {
          ...
          updatedChargers.add(
            Charger(
              ...
              isAvailable: i < availableSpots, // First 'availableSpots' are available
            ),
          );
          
          // Update charger in database
          await FirebaseFirestore.instance.collection('chargers').doc(charger.id).update({
            'is_available': i < availableSpots ? 1 : 0,
          });
        }
  ...
  Future<Reservation?> createReservation(Reservation reservation) async {
    try {
      print('Creating reservation for user ${reservation.userId} at station ${reservation.stationId}');
      ...
      String id;
      try {
        final docRef = await FirebaseFirestore.instance.collection('reservations').add(reservationMap);
        id = docRef.id;
        ...
        print('ERROR: DatabaseHelper.insertReservation is no longer available. Cannot create reservation.');
        return null;
      }
      ...
      final savedReservation = await FirebaseFirestore.instance.collection('reservations').doc(id).get();
      if (!savedReservation.exists) {
        print('ERROR: Failed to retrieve newly created reservation with ID $id');
        // Try inserting again if the first attempt didn't work
        print('Attempting to retry reservation creation...');
        final retryId = await FirebaseFirestore.instance.collection('reservations').add(reservationMap);
        print('Retry result: ID=${retryId.id}');
        
        // Check if retry worked
        final retryCheck = await FirebaseFirestore.instance.collection('reservations').doc(retryId.id).get();
        if (!retryCheck.exists) {
          ...
          try {
            ...
            await FirebaseFirestore.instance.collection('reservations').doc(emergencyId).set({
              'id': emergencyId,
            ...
            print('EMERGENCY: Created fallback reservation with ID $emergencyId');
            id = emergencyId;
          } catch (e) {
      ...
      if (reservation.chargerId != null) {
        ...
        await updateChargerAvailability(reservation.stationId, reservation.chargerId!, false);
      } else {
        // If no specific charger was selected (legacy code path), update general availability
        final stationIndex = _stations.indexWhere((s) => s.id == reservation.stationId);
        if (stationIndex >= 0) {
          final station = _stations[stationIndex];
          print('Updating station ${station.id} availability from ${station.availableSpots} to ${station.availableSpots - 1}');
          await updateStationAvailability(station.id!, station.availableSpots - 1);
        } else {
          print('Warning: Station ${reservation.stationId} not found in local stations list');
        }
      }
  ...
  Future<bool> cancelReservation(String reservationId) async {
    try {
      ...
      final reservationData = await FirebaseFirestore.instance.collection('reservations').doc(reservationId).get();
      ...
      await FirebaseFirestore.instance.collection('reservations').doc(reservationId).update({'status': 'cancelled'});
      
      // Verify the update was successful
      final updatedReservation = await FirebaseFirestore.instance.collection('reservations').doc(reservationId).get();
      ...
      if (chargerId != null) {
        ...
        print('Setting charger $chargerId at station $stationId to available after cancellation');
  ...
  Future<bool> completeReservation(String reservationId) async {
    try {
      ...
      await FirebaseFirestore.instance.collection('reservations').doc(reservationId).update({'status': 'completed'});
  ...
  Future<ChargingSession?> startChargingSession(ChargingSession session) async {
    try {
      final idRef = await FirebaseFirestore.instance.collection('charging_sessions').add(session.toMap());
      final id = idRef.id;
      
      // Update station availability if not from reservation
      if (session.reservationId == null) {
        final stationIndex = _stations.indexWhere((s) => s.id == session.stationId);
        if (stationIndex >= 0) {
          final station = _stations[stationIndex];
          await updateStationAvailability(station.id!, station.availableSpots - 1);
        }
  ...
  Future<bool> completeChargingSession(int sessionId, double energyConsumed, double amount) async {
    try {
      ...
      final sessionRef = FirebaseFirestore.instance.collection('charging_sessions').doc(sessionId.toString());
      ...
      final String? reservationId = sessionData.data()?['reservation_id'] as String?;
        ...
        if (station.id != '0') {
          ...
          for (var charger in station.chargers) {
            if (!charger.isAvailable && charger.id != null) {
              print('Making charger ${charger.id} available after charging completion');
  ...
  Future<bool> updateChargerAvailability(String stationId, String chargerId, bool isAvailable) async {
    try {
      ...
      final station = _stations[stationIndex];
      final chargerIndex = station.chargers.indexWhere((charger) => charger.id == chargerId);
      ...
      await FirebaseFirestore.instance.collection('chargers').doc(chargerId.toString()).update({
      ...
      int availableChargers = updatedChargers.where((c) => c.isAvailable).length;
      await FirebaseFirestore.instance.collection('charging_stations').doc(stationId.toString()).update({'available_spots': availableChargers});
  ...
  Future<bool> isChargerAvailableForTimeSlot(String chargerId, DateTime startTime, int durationMinutes) async {
    try {
      print('\n----- CHECKING AVAILABILITY FOR CHARGER $chargerId -----');
      print('Requested time: ${startTime.toIso8601String()} for $durationMinutes minutes');
      
      // Get all reservations for this charger regardless of status
      final reservations = await FirebaseFirestore.instance.collection('reservations').where('charger_id', isEqualTo: chargerId).get();
      print('Found ${reservations.docs.length} total reservations for this charger');
      ...
      for (var reservationDoc in reservations.docs) {
        final String status = reservationDoc.data()?['status'] as String? ?? 'unknown';
        ...
        if (status != 'confirmed') {
          print('Skipping reservation with status "$status" (ID: ${reservationDoc.id})');
          continue;
        }
        ...
        final reservationStart = DateTime.parse(reservationDoc.data()?['start_time'] as String);
        
        // Calculate reservation end time
        final reservationEnd = reservationStart.add(Duration(minutes: reservationDoc.data()?['duration'] as int));
        
        print('Checking against confirmed reservation ID: ${reservationDoc.id}');
        print('Reservation time: ${reservationStart.toIso8601String()} to ${reservationEnd.toIso8601String()}');
        ...
        if (startTime.isBefore(reservationEnd) && endTime.isAfter(reservationStart)) {
          ...
          print('Requested: ${startTime.toIso8601String()} to ${endTime.toIso8601String()}');
          print('Existing: ${reservationStart.toIso8601String()} to ${reservationEnd.toIso8601String()}');
  ...
  Future<void> refreshChargerAvailabilityData() async {
    try {
      ...
      final stationsQuery = await FirebaseFirestore.instance.collection('charging_stations').get();
      
      if (stationsQuery.docs.isEmpty) {
        print('WARNING: No stations returned from database. Using backup stations list.');
      ...
      for (final stationDoc in stationsQuery.docs) {
        try {
          ...
          final chargersQuery = await FirebaseFirestore.instance.collection('chargers').where('station_id', isEqualTo: stationDoc.id).get();
          print('Retrieved ${chargersQuery.docs.length} chargers for station ${stationDoc.id} (${stationDoc.data()?['name']})');
          
          // Convert to Charger objects
          List<Charger> chargers = chargersQuery.docs.map((doc) => Charger.fromMap(doc.data()!..['id'] = doc.id)).toList();
          ...
          print('Error loading chargers for station ${stationDoc.id} (${stationDoc.data()?['name']}): $e');
          hadErrors = true;
          
          // Add station without chargers as fallback
          _stations.add(ChargingStation.fromMap(stationDoc.data()!..['id'] = stationDoc.id));
        }
      }
      ...
      if (hadErrors && _stations.length < backupStations.length) {
        print('WARNING: Errors occurred and stations were lost. Restoring from backup.');
        _stations = backupStations;
      }
      
      // Verify that all stations were loaded
      if (_stations.isEmpty && backupStations.isNotEmpty) {
        print('CRITICAL: No stations loaded but we had stations before. Restoring from backup.');
        _stations = backupStations;
      }
      ...
      for (var station in _stations) {
        ...
        availableChargers += station.chargers.where((c) => c.isAvailable).length;
        print('Station: ${station.name} - ${station.chargers.length} chargers');
      }
      
      print('After refresh: $availableChargers out of $totalChargers chargers available');
  ...
  Future<bool> ensureChargingSessionsTableExists() async {
    try {
      ...
      if (stations.docs.isEmpty) {
        print('CRITICAL: charging_stations table does not exist! Creating it now...');
  ...
  Future<double> getTotalEnergyConsumed(String userId) async {
    try {
      ...
      if (!tableExists) {
        print('ERROR: Could not create charging_sessions table - returning cached value');
      ...
      for (var sessionDoc in allSessions.docs) {
        print('SESSION: ${sessionDoc.id} - User: ${sessionDoc.data()?['user_id']}, Energy: ${sessionDoc.data()?['energy_consumed']}, Status: ${sessionDoc.data()?['status']}');
        
        // Calculate total manually to ensure we're getting all sessions
        if (sessionDoc.data()?['user_id'] == userId && sessionDoc.data()?['status'] == 'completed') {
      ...
      try {
        final result = await db.collection('charging_sessions').where('user_id', isEqualTo: userId).where('status', isEqualTo: 'completed').get();
        
        print('Direct energy query result: ${result.docs.length}');
        
        double totalEnergy = 0.0;
        
        if (result.docs.isNotEmpty && result.docs.first.data()?['total'] != null) {
          ...
          if (totalEnergy < manualTotal) {
            print('SQL result ($totalEnergy) is less than manual calculation ($manualTotal), using manual result');
          ...
          if (totalEnergy > 0) {
            ...
            if (cachedValue > 0) {
              print('Database returned 0 kWh but we have cached value of $cachedValue kWh');
  ...
  Future<List<Map<String, dynamic>>> getChargingSessionsForUser(String userId) async {
    try {
      final sessions = await FirebaseFirestore.instance.collection('charging_sessions').where('user_id', isEqualTo: userId).get();
      return sessions.docs.map((doc) => doc.data() as Map<String, dynamic>).toList();
    } catch (e) {
  ...
  Future<bool> createTestChargingSession({
    ...
    try {
      final id = await FirebaseFirestore.instance.collection('charging_sessions').add({
        'user_id': userId,
  ...
  Future<List<Map<String, dynamic>>> _seedChargers(String stationId) async {
    ...
    for (var charger in chargers) {
      final id = await FirebaseFirestore.instance.collection('chargers').add(charger);
  ...
  Future<void> updateCachedEnergyValue(String userId, double energyValue) async {
    try {
      print('Updating cached energy value for user $userId to $energyValue kWh');
      
      // Ensure energy value is valid
      if (energyValue < 0) {
        print('WARNING: Negative energy value $energyValue is invalid, setting to 0');
        energyValue = 0.0;
      }
      ...
      if (isSignificantChange) {
        print('Significant change in energy value: $oldValue kWh -> $energyValue kWh');
      }

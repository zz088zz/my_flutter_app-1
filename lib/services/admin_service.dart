...
class AdminService with ChangeNotifier {
  ...
  Future<void> init() async {
    ...
    for (int attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        await FirebaseFirestore.instance.collection('charging_stations').limit(1).get();
        await FirebaseFirestore.instance.collection('chargers').limit(1).get();
        await FirebaseFirestore.instance.collection('users').limit(1).get();
        await FirebaseFirestore.instance.collection('transactions').limit(1).get();
  ...
  Future<void> loadAllStations() async {
    ...
    try {
      ...
      if (!await _verifyCollectionsExist()) {
        throw Exception('Cannot access required collections. Please check your admin permissions.');
      }
      ...
      for (var stationDoc in stationQuery.docs) {
        try {
          ...
          final chargerQuery = await FirebaseFirestore.instance
              .collection('chargers')
              .where('station_id', isEqualTo: stationId)
              .get();
          
          final chargers = chargerQuery.docs.map((doc) {
  ...
  Future<bool> addStation(ChargingStation station, List<Charger> chargers) async {
    _isLoading = true;
    notifyListeners();
    try {
      print('\n=== ADDING NEW STATION ===');
      ...
      stationMap['available_spots'] = chargers.where((c) => c.isAvailable).length;
      ...
      final stationRef = await FirebaseFirestore.instance.collection('charging_stations').add(stationMap);
      for (var charger in chargers) {
        charger.stationId = stationRef.id;
        await FirebaseFirestore.instance.collection('chargers').add(charger.toMap());
      }
  ...
  Future<bool> updateStation(ChargingStation station, List<Charger> chargers) async {
    _isLoading = true;
    notifyListeners();
    try {
      await init();
      ...
      stationMap['power_output'] = _getHighestPowerOutput(chargers);
      stationMap['price_per_kwh'] = _getAveragePricePerKWh(chargers);
      await FirebaseFirestore.instance.collection('charging_stations').doc(stationId).update(stationMap);
  ...
  Future<bool> deleteStation(String stationId) async {
    ...
    try {
      ...
      // Delete the station
      await FirebaseFirestore.instance.collection('charging_stations').doc(stationId).delete();
      await loadAllStations();
      return true;
    } catch (e) {
  ...
  Future<bool> addChargerToStation(String stationId, Charger charger) async {
    ...
    try {
      ...
      // Add to database
      final chargerId = await FirebaseFirestore.instance.collection('chargers').add(charger.toMap());
  ...
  Future<bool> updateCharger(Charger charger) async {
    ...
    try {
      ...
      // Update in database
      await FirebaseFirestore.instance.collection('chargers').doc(charger.id).update(charger.toMap());
  ...
  Future<bool> deleteCharger(String chargerId) async {
    ...
    try {
      ...
      await FirebaseFirestore.instance.collection('chargers').doc(chargerId).delete();
  ...
  Future<List<app_transaction.Transaction>> getAllTransactions() async {
    try {
      ...
      final transactionsData = await FirebaseFirestore.instance.collection('transactions').get();
      
      // Convert the map data to Transaction objects
      final transactions = transactionsData.docs.map((doc) {
        try {
          final data = doc.data();
          data['id'] = doc.id;
      ...
      }).where((transaction) => transaction != null).cast<app_transaction.Transaction>().toList();
  ...
  Future<void> loadAllUsers() async {
    try {
      ...
      // Fetch all admin IDs
      final adminQuery = await FirebaseFirestore.instance.collection('admins').get();
  ...
  Future<bool> disableUser(String? userId) async {
    try {
      // Make sure user ID is not null
      if (userId == null) {
        print('Cannot disable user with null ID');
        return false;
      }
      
      // Update user status to disabled in the database
  ...
  Future<bool> enableUser(String? userId) async {
    try {
      // Make sure user ID is not null
      if (userId == null) {
        print('Cannot enable user with null ID');
        return false;
      }
      ...
      await FirebaseFirestore.instance.collection('users').doc(userId).update({
        'is_active': true,
        'disabled_at': null, // Clear the disabled timestamp
      });
...
class UserService {
  // Example for SQLite or API
  static Future<List<User>> getAllUsers() async {
    // TODO: Replace with your actual database query or API call
    // For SQLite, use sqflite package to query the users table
    // For API, use http package to fetch users from backend
    // Return a List<User>
    throw UnimplementedError('Implement database fetch here');
  }
} 